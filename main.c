 ...
 *
 * This function uses the Box-Muller transform to generate a random double
 * between * min and max.
 *
...
#pragma omp critical
  ...
  return __builtin_fma(Z, stddev, mean);
}

/**
 * @brief Format a double to a string with 2 decimal places.
 *
 * @param ms the double to format
 * @return char* the formatted string
 */
char *format_ms_to_human_time(double ms) {
  static char buffer[100];
  int minutes = (int)(ms / 60000);
  int seconds = (int)((ms / 1000) - (minutes * 60));
  int milliseconds = (int)(ms - (minutes * 60000) - (seconds * 1000));

  snprintf(buffer, sizeof(buffer), "%02d:%02d.%02d", minutes, seconds,
           milliseconds / 10);
  return buffer;
}

/**
 * @brief Parse command line arguments and return the number of measurements.
 *
 * @param argc the number of arguments
 * @param argv the arguments, including the program name
 * @return long the number of measurements
 */
 ...
 *
 * We use tmpfile() to create a temporary file that is automatically
 * deleted when the program exits. This is used to store the measurements
 * for each thread, so that we can later merge them into a single file.
 *
 * @return FILE* the file pointer
 */
FILE *open_temp_file(void) {
  FILE *thread_fh = tmpfile();
  if (!thread_fh) {
    perror("Error opening temporary file. This is likely due to running out of "
           "disk space. Try running with a smaller amount of measurements.");
    exit(EXIT_FAILURE);
  }
  return thread_fh;
}
...
/**
 * @brief Write a city and its measurement to the buffer.
 *
 ...
 */
size_t write_to_buffer(char *buffer, size_t buffer_size, const char *city,
                       double measurement) {
  unsigned long written = snprintf(buffer + buffer_size, MEGABYTE - buffer_size,
                                   "%s;%.1f\n", city, measurement);
  if (written > 0 && written < MEGABYTE - buffer_size) {
...
void flush_buffer_to_file(FILE *file, char *buffer, size_t buffer_size) {
  fwrite(buffer, 1, buffer_size, file);
}

/**
 * @brief Print the progress of the current iteration.
 *
...
/**
 * @brief Write the contents of the temporary file to the output file.
 *
 * @param temp_file the temporary file handle
 * @param output_file the output file handle
 */
void write_temp_file_to_output(FILE *temp_file, FILE *output_file) {
  rewind(temp_file);
...
/**
 * @brief Generate measurements for each city and write to file.
 *
 * @param fh the output file handle
 * @param n the number of measurements to generate
...
#pragma omp for
    for (int i = 0; i < n; i++) {
      int city_index = rand() % ncities;
      double measurement = rand_nd(data[city_index].mean, 10);
    ...
    write_temp_file_to_output(thread_fh, fh);
    fclose(thread_fh);
  }
}
 ...
 *
 * @param argc the number of arguments
 * @param argv the arguments, passed in as an array of char pointers
 * @return int the exit code, 0 for success, 1 for failure
 */
int main(int argc, char **argv) {
  long n = parse_arguments(argc, argv);
  FILE *fh = open_output_file("measurements.txt");
  ...
  generate_measurements(fh, n, ncities);

  fclose(fh);
  printf("\nCreated %ld measurements in %s ms\n", n,
         format_ms_to_human_time((clock() - start) * 1000.0 / CLOCKS_PER_SEC));
  return EXIT_SUCCESS;
}
